...
class Workspace(object):
    def __init__(self, cfg):
        ...
        # print(self.agent, type(self.agent))
        # exit(0)
        ...
        self.step = 0
        self.agent.save_dir = cfg.save_dirs
        self.test_times = cfg.test_times
    ...
    def testMARL(self):
        ...
        for _ in range(self.test_times):
            ...
            obs = self.env.reset()
            while step < length_limit[self.layout_name]:
                action = self.agent.act(obs, sample=False)
                obs, rewards, done, info = self.env.step(action)
                self.video_recorder.record(self.env)
                done = self.env.overcooked.get_cooked_one()
                if done:
                    cur_success += 1
                    break
                step += 1
            self.video_recorder.save(f'test_{_}.mp4')
        print(f'{self.layout_name} success rate is: {cur_success / self.test_times}')
        


@hydra.main(config_path='config', config_name='testMARL')
